...
const dirLight = new SunLight({ timestamp: Date.UTC(2019, 7, 1, 22), color: [255, 255, 255], intensity: 1.0, _shadow: true });

const landCover: Position[][] = [[[-123.0, 49.196], [-123.0, 49.324], [-123.306, 49.324], [-123.306, 49.196]]];

type BlockProperties = { valuePerParcel: number; valuePerSqm: number; growth: number; };

function getTooltip({object}: PickingInfo<Feature<Geometry, BlockProperties>>) {
  return object && {
    html: `<div><b>Average Property Value</b></div>
...
export default function App({data = DATA_URL, mapStyle = MAP_STYLE}: { data?: string | Feature<Geometry, BlockProperties>[]; mapStyle?: string }) {
  ...
  const layers = [
    new PolygonLayer<Position[]>({ id: 'ground', data: landCover, stroked: false, getPolygon: f => f, getFillColor: [0, 0, 0, 0] }),
